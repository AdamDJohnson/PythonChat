#Chat Application - written in Python3
...
class chatapp(ttk.Frame):
    ...
    def __init__(self, config_file_path="settings.xml"):
        ttk.Frame.__init__(self, name='chatapp')
        ...
        self.master.title('Spacious Chat')
        self.focus_force()
        ...
        filemenu.add_command(label='Toggle Light & Dark Mode', command=sv_ttk.toggle_theme())
    ...
    def load_config(self, config_file_path=None):
        ...
        if config_file_path:
            ...
            for iid in [i for i in self.master.children]:
                if iid != 'chatapp' and iid not in [j.get('id') for j in self.config.iter('connection')]:
...
class ChatWindow(tk.Toplevel):
    ...
    def __init__(self, iid, displayname, address, port):
        ...
        send_button = ttk.Button(background_frame, text='Send',style="Accent.TButton", width=80,
        ...
        filemenu.add_command(label="Add Connection",
                             command=lambda: NewConnection(self.master.children['chatapp'].config,
                                                           host=self.address))
        filemenu.add_separator()
        ...
        self.display.tag_config('displayname', foreground="cyan")
    ...
    def send_and_display_msg(self, *args, **kwargs):
        ...
        if self._fetch_local_msg():
            ...
            if not socket_send_msg(self.address, self.port, self.current_local_msg):
                self.display_msg('[connection is unavailable]', ('error',))
    # END send_and_display_msg()

    def _report_update_timestamp(self):
        #Creates a timestamp that updates every 60 seconds throughout the chat to maintain accuracy in the event of needing logs
        ret = None
        _one_min_ago = datetime.datetime.now() - datetime.timedelta(minutes=1)
        if datetime.datetime.strptime(self.timestamp, self.timestamp_fmt) <= _one_min_ago:
            self.timestamp = time.strftime(self.timestamp_fmt)
            ret = self.timestamp
        return ret
...
class NewConnection(tk.Toplevel):
    ...
    def __init__(self, config_xml_tree, **kwargs):
        ...
        ttk.Label(background_frame, text="Nickname").grid()
        ...
        if conn_elem is None:
            new_conn_elem = create_elem_with_subs(self.config.getroot(), 'connection',
                                                  ...
                                                  {'hostname': hostname,
                                                   ...
                                                   'address': addr})
            self.master.children['chatapp'].populate_connection(new_conn_elem)
        self.destroy()
    # END _add()
# END NewConnection
...
class ChatRequestHandler(BaseRequestHandler):
    ...
    def handle(self):
        ...
        toaster.show_toast("New Message!",str(msg), duration = 10,
        icon_path ="https://spaciousstorage.ca/static/images/icons/unit_sizing_icon_green.svg")
    # END handle()
